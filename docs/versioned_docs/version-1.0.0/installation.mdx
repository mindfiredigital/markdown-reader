...
[Open GitHub Releases](https://github.com)
...
That warning is expected for early unsigned builds. Download only from the official GitHub Releases page linked above, then follow the steps for your operating system.
...
<div className="markdown-reader-video-frame">
  <video autoPlay loop muted playsInline aria-label="macOS Installation walkthrough">
    <source src={useBaseUrl('./video/macos-install.mp4')} type="video/mp4" />
  </video>
</div>
...
<div className="markdown-reader-video-frame">
  <video autoPlay loop muted playsInline aria-label="Linux Installation walkthrough">
    <source src={useBaseUrl('./video/linux-install.mp4')} type="video/mp4" />
  </video>
</div>
...
Some Linux desktop environments may ask you to confirm that the package or AppImage is trusted. This is expected for unsigned early builds. Install only the file you downloaded from the official GitHub Releases page.
